                            ...
                            for freeze in freezing:
                                for scenario in scenario_names:
                                    if mode == 'revalidation' or mode  == 'retraining': 
                                        if learning_rate != 0.001: # Ergibt anders keinen Sinn bei neuem Schwellenwert
                                            continue
                                    if mode == 'revalidation' and freeze == 'True':
                                        continue # Ergibt keinen Sinn bei neuem Schwellenweert.
                                    
                                    command = f'sbatch --job-name=ae{job_counter:03} evaluation_ae.job {version} {scenario} {config} {result_path} {learning_rate} {mode} {freeze} {validation_mode}'
                                    os.system(command)
